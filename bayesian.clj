...
(defn compress
  "Run length encoding. (Based on version in book ANSI Common Lisp.)"
  [x]
  (vec (compr [] (first x) 1 (rest x)))) 

(defn word-count
  "Creates a hash-map of word counts."
  [lst]
  (apply merge (-> lst sort compress)))

(defn parse-file
  "Extracts words from file for counting."
  [file]
  (word-count 
...
(def words (ref (load-file "/tmp/words.clj")))
